<?php 
	...
	class WordOfMouthController extends Controller{
		...
		public function index()
		{
			...
			if( I('get.type') !=null ){
				redirect( U('word_of_mouth/index/id/'.I('get.id').'/order/'.I('get.type') ) );
			}
			$this->assign($info);
			$this->display();
		}
		...
		public function toAdd()
		{
			...
			if( $res = $praise->handle() ){
				$this->success('口碑已发表成功',U('Home/word_of_mouth/myPraise/uid/'.$_SESSION['home_user_info']['user_id']));
			}
		}
		...
		public function myPraise()
		{
			...
			// echo '</pre>';
			if( I('get.type') !=null ){
				redirect(U('word_of_mouth/myPraise/uid/'.I('get.uid').'/order/'.I('get.type')));
			}
